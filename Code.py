...
# Using highly visible arrow indicators directly in the index names
INV_ROWS = [
    "Beyet (1)",
    "Achat",
    "Vente (1)",
    "Reste (1)",
    "➡️ DIFFÉRENCE (1) 🌟",  # Clean text highlight for calculation row 1
    "Reste (2)",
    "Plus",
    "Vente (2)",
    "Beyet (2)",
    "➡️ DIFFÉRENCE (2) 🌟"   # Clean text highlight for calculation row 2
]
...
st.caption("Enter your inventory logs below. The difference rows calculate automatically.")

working_inv_df = st.session_state.inventory_data.copy()

# Math execution loop with flipped formula signs calculated live on backend
for col in INV_COLUMNS:
    try:
        ...
        working_inv_df.at["➡️ DIFFÉRENCE (1) 🌟", col] = diff1
        ...
        working_inv_df.at["➡️ DIFFÉRENCE (2) 🌟", col] = diff2
    except KeyError:
        pass

# Render editable grid without using internal pandas styling to maintain stability
edited_inv_df = st.data_editor(
    working_inv_df,
    use_container_width=True,
    disabled=["➡️ DIFFÉRENCE (1) 🌟", "➡️ DIFFÉRENCE (2) 🌟"], # Keep formula rows locked
